...
The first thing to ask is: if we control the env variables, how we can achieve RCE?
...
Now this would be great if the sessions were stored on disk. As it turns out `express-session` saves the sessions in memory so we cross of this possibility.
