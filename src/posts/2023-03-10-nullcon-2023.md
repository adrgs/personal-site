...
![](https://i.imgur.com/o4SasES.jpg)

After raking:

![](https://i.imgur.com/XsEEpY2.jpg)
...
We can collect three $(N_i, c_i)$ pairs when $e = 3$. Using CRT, we can calculate $c$ such that $c \equiv c_i \mod N_i$. Then $m = c^{1/3}$.
...
We can collect three $(N_i, c_i, a_i)$ pairs, when $e = 3$, such that $(a_i \times 256^{L} + m)^e = c_i\mod N_i$, where $L$ is a length of `invitation`. This $m$ can be recovered by Håstad's broadcast attack.
...
However this STOPPED working for this challenge after they uploaded `randrevengerevenge`, which should not be the case.
