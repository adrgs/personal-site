...
Umarshal data => add pyc header => unmarshal data again and use dis to get bytecode => reimplement algorithm
...
We can get the first flag just by changing file0.enc and file0.dec to file1.enc and file1.dec

Flag: **CTF{0B613DEDCC13EF2AEC1716633AEA97D876A5BAED5658B61C07F698BE5CB8DBD2}**

For the second flag, we must recover the key and the IV and then change the encryption from AES CBC to AES CTR. Notice how the length 78 is 69 (flag) + 9 (file2.enc)
...
Service on UDP port 768 contains the flag:
...
UPX compressed file with messed up headers, used this tutorial to recover the original binary: https://infosecwriteups.com/how-to-unpack-upx-packed-malware-with-a-single-breakpoint-4d3a23e21332

In the decompressed binary there's an interesting part that xors two strings from memory.
